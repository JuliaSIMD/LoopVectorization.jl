...
function add_constant_vload!(
  ...
  vloadcall =
    use_getindex ? Expr(:call, Base.getindex, name(mpref)) :
    Expr(:call, lv(:_vload), mpref.mref.ptr)
  ...
  if nindices > 0
    ...
    if use_getindex
      append!(
        ...
        mem_offset(
          op,
          UnrollArgs(dummyloop, dummyloop, dummyloop, 0, 0, 0),
          fill(false, nindices),
          true,
          ls,
          false,
